...
const MyAppointments = () => {
    const { backendUrl, PToken, appointments, getPatientAppointments, slotDateFormat } = useContext(AppContext)

    const cancelAppointment = async (appointmentId) => {
        try {
            const { data } = await axios.post(backendUrl + '/api/pat/cancel-appointment', { appointmentId }, { headers: { PToken } })
            if (data.success) {
            ...
            <div>
                {appointments && appointments.map((item, index) => (
                    <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                        <div>
                            <img className='w-32 bg-indigo-50' src={item.docData?.image || '/default-doctor.png'} alt="" />
                        </div>
