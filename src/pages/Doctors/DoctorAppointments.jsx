          ...
          appointments.reverse().map((item, index) => (
            <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-100' key={index}>
              ...
              <div className='flex items-center gap-2'>
                <img className='w-8 rounded-full' src={item.userData.image || item.patData.image} alt="" /> <p>{item.userData.name || item.patData.name} :<br/>{item.userData.idnumber || item.patData.idnumber}</p>
              </div>
              <div>
              ...
              <p className='max-sm:hidden'>{calculateAge(item.userData.dob || item.userData.dob)}</p>
